...
* [`src/clojure_tutorials/`](src/clojure_tutorials/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/clojure_tutorials/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `clojure-tutorials` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/clojure_tutorials/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns clojure-tutorials.example
  (:require [clojure-tutorials.config :as config])
